...
class TownState(LockInteractionMixin, InventoryMixin, BaseState):
    ...
    def handle_input(self, events, keys_pressed):
        ...
        # player can pan across the town/interior map and review
        # previously explored tiles under the fog-of-war mask.
        # Panning does NOT advance game time. Releasing Shift
        # snaps the camera back to the party on the next frame.
    ...
    def _enter_interior(self, interior_name, door_col, door_row):
        ...
        # Track exit positions from interior tile flags
        self._interior_exit_positions = set()
        exit_positions = []
        for pos_key, td in interior.get("tiles", {}).items():
            parts = pos_key.split(",")
            c, r = int(parts[0]), int(parts[1])
            tile_flags = td.get("flags", {})
            if tile_flags.get("to_town"):
