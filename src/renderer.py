...
class Renderer(CombatEffectRendererMixin):
    ...
    def draw_town_u3(self, party, town_data, message="",
        ...
        self._u3_text("[H] HELP", SCREEN_WIDTH - 110, row1_y + 2,
                      (68, 68, 255), font=self.font_small)
    ...
    def _u3_draw_door_interact(self, interact, off_c, off_r, ts):
        ...
        self._u3_text("[ENTER] SELECT  [ESC] LEAVE",
                       panel_x + 10, oy + 2, self._U3_HINT, self.font_small)
    ...
    # ── (dungeon fog of war — moved to src/lighting.py) ──

    # ── (infravision + Galadriel tints — moved to src/lighting.py) ──
    ...
    _U3_BLACK  = (0, 0, 0)
    _U3_BLUE   = (68, 68, 255)    # style-guide canonical border/hint blue
    ...
    def _u3_draw_orc_combat_sprite(self, monster, ax, ay, ts, col, row):
        ...
        Multi-tile monsters (battle_scale > 1) are drawn across an NxN
        block of tiles, with the sprite scaled to fill the full area.
        """
        scale = getattr(monster, 'battle_scale', 1) or 1
        # Centre of the NxN block
        cx = ax + col * ts + (ts * scale) // 2
        cy = ay + row * ts + (ts * scale) // 2
        full_size = ts * scale

        sprite = self._get_monster_sprite(monster)
        if sprite:
            # Scale sprite to fill NxN tiles
            if scale > 1:
                cache = getattr(self, '_scaled_monster_cache', None)
                if cache is None:
                    cache = self._scaled_monster_cache = {}
                key = (id(sprite), full_size)
                scaled = cache.get(key)
                if scaled is None:
                ...
                sprite_draw = sprite
            sx = cx - sprite_draw.get_width() // 2
            sy = cy - sprite_draw.get_height() // 2
            self.screen.blit(sprite_draw, (sx, sy))
        else:
            mc = monster.color
            bw = int(16 * scale)
            bh = int(14 * scale)
            ...
            head_h = int(7 * scale)
            pygame.draw.rect(self.screen, mc,
                             pygame.Rect(cx - head_w // 2,
                                         cy - bh // 2 - head_h,
                                         head_w, head_h))
            eye_s = max(1, int(3 * scale))
            eye_y = cy - bh // 2 - head_h + int(2 * scale)
            pygame.draw.rect(self.screen, self._U3_WHITE,
                             pygame.Rect(cx - int(4 * scale), eye_y,
                                         eye_s, eye_s))

        # HP bar below the monster sprite
        hp_ratio = max(0.0, min(1.0, monster.hp / monster.max_hp)) if monster.max_hp > 0 else 0.0
        bar_w = max(full_size - 4, 20)
        bar_h = 3
        bar_x = cx - bar_w // 2
        bar_y = cy + full_size // 2 + 2
    ...
    def _u3_draw_monster_sprite(self, monster, ax, ay, ts, col, row):
        ...
        if sprite:
            ...
            # Fallback: blocky monster scaled to NxN tiles
            mc = monster.color
            W = self._U3_WHITE
            hw = full_size // 2
            bw = int(16 * scale)
            ...
            pygame.draw.rect(self.screen, W,
                             pygame.Rect(cx - int(4 * scale), eye_y,
                                         eye_s, eye_s))
            pygame.draw.rect(self.screen, W,
                             pygame.Rect(cx + int(1 * scale), eye_y,
                                         eye_s, eye_s))
    ...
    # ── Map Editor hub screen ─────────────────────────────────────

    def draw_map_editor_hub(self, data):
        """Draw the top-level Map Editor hub screen.

        A simple section browser with folder navigation and field editing.
        """
        ...
        nav_depth = data.get("nav_depth", 0)
        ...
        if nav_depth > 0 and folder_label:
            self._u3_text("MAP EDITOR", 16, 6,
                          (120, 100, 80), fs)
            self._u3_text(folder_label.upper(), 16, 22,
                          self._U3_ORANGE, fm)
        else:
            self._u3_text("MAP EDITOR", 16, 14,
                          self._U3_ORANGE, fm)

        # ── Field editing mode ──
        if level == 1:
            self._draw_meh_fields(data, header_h)
            return
        ...
        if n == 0:
            self._u3_text("No map templates yet.",
                          SCREEN_WIDTH // 2 - 80,
                          SCREEN_HEIGHT // 2 - 10,
                          (140, 140, 160), fs)
        else:
            sc = max(0, cursor - max_vis + 1)
            for i in range(sc, min(sc + max_vis, n)):
                y = list_y0 + (i - sc) * row_h
                sec = sections[i]
                is_sel = (i == cursor)

                if is_sel:
                    bar = pygame.Surface(
                        (SCREEN_WIDTH - 80, row_h - 4), pygame.SRCALPHA)
                    bar.fill((255, 200, 60, 40))
                    self.screen.blit(bar, (list_x, y))
                    pygame.draw.rect(self.screen, (200, 160, 60),
                                     (list_x, y,
                                      SCREEN_WIDTH - 80, row_h - 4), 1)

                label = sec.get("label", "?")
                subtitle = sec.get("subtitle", "")
                is_folder = bool(sec.get("folder"))

                # Label
                nc = (255, 255, 100) if is_sel else (200, 200, 220)
                ...
                # Subtitle
                if subtitle:
                    self._u3_text(subtitle, list_x + 30, y + 24,
                                  (120, 120, 140), fs)

                # Folder arrow
                if is_folder and is_sel:
                    self._u3_text("\u25B6", SCREEN_WIDTH - 100, y + 8,
                                  (200, 160, 60), f)
    ...
    def _draw_meh_fields(self, data, header_h):
        ...
        fi = data.get("field_cursor", 0)
        buf = data.get("field_buffer", "")
        n = len(fields)

        row_h = 36
        field_x = 60
        field_y0 = header_h + 20

        for i in range(n):
            y = field_y0 + i * row_h
            if y + row_h > SCREEN_HEIGHT - 50:
                break

            label = fields[i][0]
            value = fields[i][2]
            ftype = fields[i][3] if len(fields[i]) > 3 else "text"
            editable = fields[i][4] if len(fields[i]) > 4 else True
            is_sel = (i == fi)

            if ftype == "section":
                # Section header (not editable)
                self._u3_text(label, field_x, y + 8,
                              (120, 120, 160), fm)
                continue
    ...
    def draw_features_screen(self, state):
        ...
        # ── Header ──
        self._u3_text("Edit Game", SCREEN_WIDTH // 2 - 80,
        ...
        if level == 0:
            ...
            if selected_cat == "Modules":
                ...
                self._u3_text("Edit Game", right_x + 16,
                              panel_y + 12, self._U3_WHITE, f)
                dy = panel_y + 44
                self._u3_text(
            ...
            if ed == "spells" or ed is None:
                ...
                if meh_editor_active and meh_editor_data:
                    ...
                    self._draw_map_editor_fullscreen(meh_editor_data)
                else:
                    # Hub browser
                    hub_data = {
                        "sections": meh_sections or [],
                        "cursor": meh_cursor,
                        "scroll": meh_scroll,
                        "nav_depth": meh_nav_depth,
                        "folder_label": meh_folder_label,
                        "level": meh_level,
                        "fields": meh_fields or [],
    ...
    # ── Dungeon editor rendering ─────────────────────────────────
    #
    # Hierarchy:  level 0 = dungeon list, 1 = dungeon sub, 2 = settings/level list,
    #             3 = level sub, 4 = encounter list, 5 = encounter editor

    def draw_dungeon_editor_panel(self, rx, ry, rw, rh, dg):
        """Draw the dungeon editor panel inside the module editor."""
        fm = self.font_med
        fs = self.font_small
        f = self.font

        dungeons = dg.get("dungeons") or []
        cursor = dg.get("cursor", 0)
        scroll = dg.get("scroll", 0)
        level = dg.get("level", 0)
        row_h = 36

        # ── Map editor fullscreen ──
        if dg.get("editor_active") and dg.get("editor_data"):
            self._draw_map_editor_fullscreen(dg["editor_data"])
            return
        ...
        # ── Level 0: Dungeon list ──
        if level == 0:
            self._u3_text("DUNGEONS", rx + 16, ry + 12,
                          self._U3_ORANGE, f)
            ly = ry + 44
            content_h = rh - 80
            max_vis = content_h // row_h
            ...
            if len(dungeons) == 0:
                self._u3_text("No dungeons yet. Press Ctrl+N to add.",
                              rx + 20, ly + 10, (140, 140, 160), fm)
            else:
                for vi in range(max_vis):
                    i = scroll + vi
                    ...
                    levels_arr = dungeon.get("levels", [])
                    n_levels = len(levels_arr)
                    n_enc = sum(len(lv.get("encounters", []))
                                for lv in levels_arr)
                    self._u3_text(f"{prefix}{name}",
                                  rx + 10, y, color, fm)
                    self._u3_text(
                        f"  {n_levels} level{'s' if n_levels != 1 else ''}"
                        f"  {n_enc} encounters",
                        rx + 10, y + 18, (140, 140, 160), fs)

            # Naming overlay (dungeons)
            if dg.get("naming") and dg.get("naming_target") == "dungeon":
                self._draw_naming_overlay(
                    dg.get("name_buf", ""),
                    "New Dungeon Name:" if dg.get("naming_is_new")
                    else "Rename Dungeon:",
                    fm, f)
        ...
        # ── Level 1: Dungeon sub-screen (Settings | Levels) ──
        elif level == 1:
            dungeon = dungeons[cursor] if 0 <= cursor < len(dungeons) else {}
            dungeon_name = dungeon.get("name", "Unnamed")
            self._u3_text(f"DUNGEON: {dungeon_name}", rx + 16, ry + 12,
                          self._U3_ORANGE, f)
            ly = ry + 50
            sub_items = dg.get("sub_items") or ["Settings", "Levels"]
            sub_cursor = dg.get("sub_cursor", 0)
            for i, item in enumerate(sub_items):
                selected = (i == sub_cursor)
                y = ly + i * 40
                if selected:
        ...
        # ── Level 2: Settings fields OR Level list ──
        elif level == 2:
            sub_cursor = dg.get("sub_cursor", 0)
            if sub_cursor == 0:
                self._draw_dungeon_settings(
                    dg, rx, ry, rw, rh, fm, fs, f)
            elif sub_cursor == 1:
                self._draw_dungeon_level_list(
                    dg, rx, ry, rw, rh, fm, fs, f)

        # ── Level 3: Level sub-screen (Edit Map | Encounters) ──
        elif level == 3:
            self._draw_dungeon_level_sub(
                dg, rx, ry, rw, rh, fm, fs, f)
    ...
    def _draw_dungeon_settings(self, dg, rx, ry, rw, rh, fm, fs, f):
        """Draw the dungeon settings field editor."""
        fields = dg.get("fields") or []
    ...
    def _draw_dungeon_level_list(self, dg, rx, ry, rw, rh, fm, fs, f):
        """Draw the level list within a dungeon."""
        levels = dg.get("level_list") or []
        cursor = dg.get("level_cursor", 0)
        scroll = dg.get("level_scroll", 0)
        row_h = 36

        dungeons = dg.get("dungeons") or []
        dc = dg.get("cursor", 0)
        dungeon = dungeons[dc] if 0 <= dc < len(dungeons) else {}
        dungeon_name = dungeon.get("name", "Dungeon")

        self._u3_text(f"LEVELS — {dungeon_name}", rx + 16, ry + 12,
                      self._U3_ORANGE, f)
        ly = ry + 44
        content_h = rh - 80
        max_vis = content_h // row_h
        ...
        if len(levels) == 0:
            self._u3_text("No levels yet. Press Ctrl+N to add.",
                          rx + 20, ly + 10, (140, 140, 160), fm)
        else:
            for vi in range(max_vis):
                i = scroll + vi
                ...
                lw = lv.get("width", 0)
                lh = lv.get("height", 0)
                n_enc = len(lv.get("encounters", []))
                has_map = bool(lv.get("tiles"))
                map_tag = "map" if has_map else "no map"
                self._u3_text(f"{prefix}{name}",
                              rx + 10, y, color, fm)
                self._u3_text(
                    f"  {lw}x{lh}  {n_enc} enc  ({map_tag})",
                    rx + 10, y + 18, (140, 140, 160), fs)

        # Naming overlay (levels)
        if dg.get("naming") and dg.get("naming_target") == "level":
            self._draw_naming_overlay(
                dg.get("name_buf", ""),
                "New Level Name:" if dg.get("naming_is_new")
                else "Rename Level:",
                fm, f)
    ...
    def _draw_dungeon_level_sub(self, dg, rx, ry, rw, rh, fm, fs, f):
        """Draw the level sub-screen selector (Edit Map | Encounters)."""
        levels = dg.get("level_list") or []
        lc = dg.get("level_cursor", 0)
        level = levels[lc] if 0 <= lc < len(levels) else {}
        level_name = level.get("name", "Unnamed")

        self._u3_text(f"LEVEL: {level_name}", rx + 16, ry + 12,
                      self._U3_ORANGE, f)
        ly = ry + 50
        sub_items = dg.get("level_sub_items") or ["Edit Map", "Encounters"]
        sub_cursor = dg.get("level_sub_cursor", 0)
        for i, item in enumerate(sub_items):
            selected = (i == sub_cursor)
            y = ly + i * 40
            if selected:
    ...
    def _draw_dungeon_encounter_list(self, dg, rx, ry, rw, rh, fm, fs, f):
        """Draw the encounters list within a level."""
        encounters = dg.get("encounter_list") or []
        cursor = dg.get("encounter_cursor", 0)
        scroll = dg.get("encounter_scroll", 0)
        row_h = 36
    ...
    def draw_module_screen(self, modules, cursor, active_path,
        ...
        edit_scroll      : scroll offset for field list
        edit_level       : 0 = section browser, 1 = field editor
        edit_sections    : list of section dicts
        ...
        if edit_mode:
            ...
            elif edit_level in (8, 9, 10, 11, 12, 13) and dungeon_data:
    ...
    def _draw_map_editor_fullscreen(self, data):
        """Draw the overview map tile editor using the unified renderer."""
        from src.map_editor_renderer import draw_map_editor
        draw_map_editor(self, data)
        ...
        if max_scroll > 0:
            if scroll > 0:
                self._u3_text("^ UP ^", panel_x + panel_w // 2 - 24,
                              panel_y + 2, self._U3_HINT, self.font_small)
            if scroll < max_scroll:
                self._u3_text("v DOWN v", panel_x + panel_w // 2 - 30,
                              panel_y + panel_h - 32, self._U3_HINT,
                              self.font_small)
    ...
    # ── (overworld darkness — moved to src/lighting.py) ──
    ...
    def draw_char_create_screen(self, game):
        ...
        # ── GENDER SELECTION ──
            ...
            for lbl, val in [("NAME:", game._cc_name),
                ...
                sy += 24
            self._u3_text("[UP/DOWN] SELECT   [ENTER] NEXT   [ESC] BACK",
                          cx, cy + ph - 60, self._U3_HINT, f_hint)
        ...
        # ── TILE SELECTION ──
            ...
            if scroll_row > 0:
                self._u3_text("^ MORE ^",
                              ...
                              grid_y - 10, self._U3_HINT, f_hint)
        ...
        # ── DONE ──
            ...
            self._u3_text(
                ...
                cx, cy + 70, self._U3_HINT, f_hint)
            self._u3_text(
                f"ROSTER: {len(game.party.roster)}/{game.party.MAX_ROSTER}",
                cx, cy + 100, self._U3_HINT, f_hint)
    ...
    def draw_form_party_screen(self, game):
        ...
        if scroll > 0:
            self._u3_text("^ MORE ^", roster_x + roster_w // 2 - 30,
                          roster_y - 4, self._U3_HINT, self.font_small)
        ...
        self._u3_text(
            ...
            px + 4, hint_y, self._U3_HINT, self.font_small)
