...
A module is a folder under `modules/` containing the JSON files that define a module's content. The exact list of files is documented in the data dictionary at `docs/data_dictionary/`; in practice a module includes monsters, items, spells, effects, potions, encounters, spawn points, counters, towns, and starting party state. Some files are global (the tile catalog `tile_defs.json`, foundational systems like classes and races) and live in `data/` rather than per-module — those are platform-level and shared across every module.
...
**Which JSON files are per-module versus global?** The data dictionary captures what each file does today, but v2 needs an explicit decision about which ones live in `modules/<id>/` and which live in `data/`. Strong candidates for global: `tile_defs.json`, `classes/*.json`, `races.json`. Strong candidates for per-module: `monsters.json`, `items.json`, `spells.json`, `effects.json`, `potions.json`, `encounters.json`, `spawn_points.json`, `counters.json`, `party.json`, the towns and dungeons data. Tile defs in particular might want to be promoted to per-module later if modules need custom tiles — flag this as a soft decision.

**`module.json` schema.** What fields exactly? At minimum: `id`, `title`, `description`, `author`, `version`, `gameVersion` (min compatible engine version). Possibly: `tags`, `previewImage`, `startingNotes`, license info.
...
**Editor scope for v1 of v2.** The Python editor does a lot. The web editor will not start at parity. Pick a minimum viable feature set — likely: edit monsters, items, spells, effects, encounters, spawn points, towns, party. Map editing (the big one) probably needs its own design pass and may not be in the first cut.
...
1. Decide the per-module vs. global file split. This unblocks the directory layout.
2. Write the `module.json` schema doc as an addition to `docs/data_dictionary/`.
3. Build the module index generator script (`web/scripts/build-module-index.mjs`) and wire it as `prebuild`.
4. Refactor the existing `the_dragon_of_dagorn` module into the new layout. This validates the design against real content and produces the first reference module.
5. Implement `ModuleSource` and replace the hardcoded `ACTIVE_MODULE` constant with module-picker UI on the title screen.
6. Implement `SaveProvider` and rework the existing save/load paths through it. Add export/import buttons.
7. Build the editor. Start with the smallest reasonable scope (monsters, items) to validate the editor → draft → game-test loop end-to-end, then expand to the rest of the per-module files.
